...
static int load_documents_from_dir(const char *dirname,
				   Document_Vector *docs)
{
...
int save_doc_to_json(Document_Vector docs, const char *path)
{
	...
	for (size_t j = 0; j < src->tf.alloc; ++j) {
	    ...
	    cJSON_AddNumberToObject(doc, entry->term.data,
				    (double) entry->freq);
	}
    }
...
int main(void)
{
    ...
    qsort(results.data, results.size, sizeof(*results.data),
	  search_result_cmp);
